 ...
 *  封装的主要API：
 *  http://dev.wireless.taobao.net/mediawiki/index.php?title=Mtop.sys.newDeviceI
...
/** 将登陆用户和手机绑定在一起.
 *
 *  @param  sessionID       淘系用户传入sid
 *  @param  sToken          非淘系用户传入用户登陆标示
 ...
 */
- (void)bindUserIntoPushCenterWithSessionID:(NSString *)sessionID
 ...
 *  @param  failBlock       绑定失败后，回调block
 */
- (void)unbindUserIntoPushCenterWithPushUserToken:(NSString *)pushUserToken
                                         userInfo:(NSDictionary *)userInfo
