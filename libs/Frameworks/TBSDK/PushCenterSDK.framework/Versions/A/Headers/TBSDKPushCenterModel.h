...
/** 操作失败，回调block */
typedef void(^ PushCenterModelFailureBlock)(TBSDKPushCenterModel *, TBSDKErrorResponse *);
...
/** 错误描述对象 */
@property (nonatomic, strong) TBSDKErrorResponse                                *error;

/** 回调对象 */
@property (nonatomic, unsafe_unretained) id<TBSDKPushCenterDataModelDelegate>   delegate;

/** 回调信息存储对象 */
@property (nonatomic, unsafe_unretained) TBSDKPushCenterContext                 *context;
...
- (TBSDKErrorResponse *)errorWithErrorCode:(NSString *)errorCode
                                       msg:(NSString *)msg
...
- (void)pushCenterDataModelDidFail:(TBSDKPushCenterModel *)pushCenterDataModel
                             error:(TBSDKErrorResponse *)error;
