...
int EncodeWAVEFileToAMRFile(const char *pchWAVEFilename, const char *pchAMRFileName, int nChannels, int nBitsPerSample) {
    ...
    while (1) {
        ...
        bytes += byte_counter;
        fwrite(amrFrame, sizeof (unsigned char), (size_t) byte_counter, fpamr);
    }
...
int ReadAMRFrameFirst(FILE *fpamr, unsigned char frameBuffer[], int *stdFrameSize, unsigned char *stdFrameHeader) {
    ...
    *stdFrameSize = caclAMRFrameSize(*stdFrameHeader);
    ...
    fread(&(frameBuffer[1]), 1, ((size_t) *stdFrameSize - 1) * sizeof(unsigned char), fpamr);
    if (feof(fpamr)) return 0;
...
//    memset(frameBuffer, 0, sizeof(frameBuffer));
    ...
    bytes = fread(&(frameBuffer[1]), 1, ((size_t) stdFrameSize - 1) * sizeof(unsigned char), fpamr);
    if (feof(fpamr)) {
        return 0;
    }
...
static size_t TBIUBufferRead(void *restrict dest, size_t size, TBIU_BUFFER *restrict buffer) {
    size_t realSize = 0;
    if (size > buffer->length - buffer->pos) {
        realSize = buffer->length - buffer->pos;
    } else {
        realSize = size;
    }
    if (realSize > 0) {
        memcpy(dest, buffer->data + buffer->pos, realSize);
    }
    buffer->pos += realSize;
    return realSize;
}

static size_t TBIUBufferSeek(size_t size, TBIU_BUFFER *restrict buffer) {
    size_t realSize = 0;
    if (size > buffer->length - buffer->pos) {
        realSize = buffer->length - buffer->pos;
    } else {
...
static bool TBIUBufferIsEof(TBIU_BUFFER *restrict buffer) {
    return buffer->pos >= buffer->length;
}


static TBIU_BUFFER TBIUMakeBuffer(NSData *data) {
    TBIU_BUFFER buffer;
...
#pragma mark - NSData Decode AMR->WAV
void WriteWAVEDataHeader(NSMutableData *data, int nFrame) {
    char tag[10] = "";
...
int ReadAMRDataFrameFirst(TBIU_BUFFER *buffer, unsigned char frameBuffer[], int *stdFrameSize,
        unsigned char *stdFrameHeader) {
    size_t bytes = 0;
    // 先读帧头
    bytes = TBIUBufferRead(stdFrameHeader, 1 * sizeof(unsigned char), buffer);
    if (bytes == 0) return 0;
    ...
    *stdFrameSize = caclAMRFrameSize(*stdFrameHeader);
    ...
    // 读首帧
    frameBuffer[0] = *stdFrameHeader;
    bytes = TBIUBufferRead(&(frameBuffer[1]), (size_t) (1 * (*stdFrameSize - 1) * sizeof(unsigned char)), buffer);
    if (bytes == 0) return 0;
...
// 返回值: 0-出错; 1-正确
int ReadAMRDataFrame(TBIU_BUFFER *buffer, unsigned char frameBuffer[], int stdFrameSize, unsigned char stdFrameHeader) {
    int bytes = 0;
    unsigned char frameHeader; // 帧头
...
//    memset(frameBuffer, 0, sizeof(frameBuffer));
    ...
    // 如果是坏帧(不是标准帧头)，则继续读下一个字节，直到读到标准帧头
    while (1) {
        bytes = TBIUBufferRead(&frameHeader, 1 * sizeof(unsigned char), buffer);
        if (bytes == 0)
            return 0;
    ...
    // 读该帧的语音数据(帧头已经读过)
    frameBuffer[0] = frameHeader;
    bytes = TBIUBufferRead(&(frameBuffer[1]), 1 * ((size_t) stdFrameSize - 1) * sizeof(unsigned char), buffer);
    if (bytes == 0) {
        return 0;
    }
...
NSData *DecodeAMRToWAVE(NSData *amrData) {
    ...
    TBIU_BUFFER armBuffer = TBIUMakeBuffer(amrData);

    TBIUBufferRead(magic, sizeof(char) * strlen(AMR_MAGIC_NUMBER), &armBuffer);
    if (strncmp(magic, AMR_MAGIC_NUMBER, strlen(AMR_MAGIC_NUMBER))) {
        return nil;
    }


    WriteWAVEDataHeader(wavData, nFrameCount);
    ...
    ReadAMRDataFrameFirst(&armBuffer, amrFrame, &stdFrameSize, &stdFrameHeader);

    // 解码一个AMR音频帧成PCM数据
    Decoder_Interface_Decode(destate, amrFrame, pcmFrame, 0);
    nFrameCount++;

    [wavData appendBytes:pcmFrame
                  length:sizeof(short) * PCM_FRAME_SIZE];

    // 逐帧解码AMR并写到WAVE文件里
    while (1) {
        memset(amrFrame, 0, sizeof(amrFrame));
        memset(pcmFrame, 0, sizeof(pcmFrame));
        if (!ReadAMRDataFrame(&armBuffer, amrFrame, stdFrameSize, stdFrameHeader)) break;
    ...
    WriteWAVEDataHeader(wavHeader, nFrameCount);
    if (wavData.length >= wavHeader.length) {
        [wavData replaceBytesInRange:NSMakeRange(0, wavHeader.length)
                           withBytes:wavHeader.bytes];
    }

    return wavData;
}
#pragma mark - NSData Decode WAV->AMR

void SkipDataToPCMAudioData(TBIU_BUFFER *wavBuffer) {
    RIFFHEADER riff;
    FMTBLOCK fmt;
    XCHUNKHEADER chunk;
    WAVEFORMATX wfx;
    ...
    // 1. 读RIFF头
    TBIUBufferRead(&riff, 1 * sizeof(RIFFHEADER), wavBuffer);

    // 2. 读FMT块 - 如果 fmt.nFmtSize>16 说明需要还有一个附属大小没有读
    TBIUBufferRead(&chunk, 1 * sizeof(XCHUNKHEADER), wavBuffer);
    if (chunk.nChunkSize > 16) {
        TBIUBufferRead(&wfx, 1 * sizeof(WAVEFORMATX), wavBuffer);
    }
    else {
        memcpy(fmt.chFmtID, chunk.chChunkID, 4);
        fmt.nFmtSize = chunk.nChunkSize;
        TBIUBufferRead(&fmt.wf, 1 * sizeof(WAVEFORMAT), wavBuffer);
    }

    // 3.转到data块 - 有些还有fact块等。
    while (1) {
        size_t bytes = TBIUBufferRead(&chunk, 1 * sizeof(XCHUNKHEADER), wavBuffer);
        if (bytes == 0) {
            break;
        }
        if (!memcmp(chunk.chChunkID, "data", 4)) {
        ...
        // 因为这个不是data块,就跳过块数据
        TBIUBufferSeek((size_t) (chunk.nChunkSize * SEEK_CUR), wavBuffer);
    }
}

// 从WAVE文件读一个完整的PCM音频帧
// 返回值: 0-错误 >0: 完整帧大小
int ReadPCMDataFrame(short speech[], TBIU_BUFFER *wavBuffer, int nChannels, int nBitsPerSample) {
    int nRead = 0;
    int x = 0, y = 0;
//	unsigned short ush1=0, ush2=0, ush=0;
    ...
    if (nBitsPerSample == 8 && nChannels == 1) {
        nRead = TBIUBufferRead(pcmFrame_8b1, (size_t) ((nBitsPerSample / 8) * PCM_FRAME_SIZE* nChannels), wavBuffer);
        for (x = 0; x < PCM_FRAME_SIZE; x++) {
            speech[x] = (short) ((short) pcmFrame_8b1[x] << 7);
        }
    }
    else if (nBitsPerSample == 8 && nChannels == 2) {
        nRead = TBIUBufferRead(pcmFrame_8b2, (size_t) ((nBitsPerSample / 8) * PCM_FRAME_SIZE* nChannels), wavBuffer);
        for (x = 0, y = 0; y < PCM_FRAME_SIZE; y++, x += 2) {
            // 1 - 取两个声道之左声道
            speech[y] = (short) ((short) pcmFrame_8b2[x + 0] << 7);
            // 2 - 取两个声道之右声道
        ...
        nRead = TBIUBufferRead(pcmFrame_16b1, (size_t) ((nBitsPerSample / 8) * PCM_FRAME_SIZE* nChannels), wavBuffer);
        for (x = 0; x < PCM_FRAME_SIZE; x++) {
            speech[x] = (short) pcmFrame_16b1[x + 0];
        }
    }
    else if (nBitsPerSample == 16 && nChannels == 2) {
        nRead = TBIUBufferRead(pcmFrame_16b2, (size_t) ((nBitsPerSample / 8) * PCM_FRAME_SIZE* nChannels), wavBuffer);
        for (x = 0, y = 0; y < PCM_FRAME_SIZE; y++, x += 2) {
            //speech[y] = (short)pcmFrame_16b2[x+0];
            speech[y] = (short) ((int) ((int) pcmFrame_16b2[x + 0] + (int) pcmFrame_16b2[x + 1])) >> 1;
        }
...
NSData *EncodeWAVEToAMR(NSData *wavData, int nChannels, int nBitsPerSample) {
    NSMutableData *armData = [NSMutableData data];
    ...
    TBIU_BUFFER wavBuffer = TBIUMakeBuffer(wavData);

    /* write magic number to indicate single channel AMR file storage format */
    [armData appendBytes:AMR_MAGIC_NUMBER
                  length:strlen(AMR_MAGIC_NUMBER)];

    /* skip to pcm audio data*/
    SkipDataToPCMAudioData(&wavBuffer);

    enstate = Encoder_Interface_init(dtx);

    while (1) {
        // read one pcm frame
        if (!ReadPCMDataFrame(speech, &wavBuffer, nChannels, nBitsPerSample)) break;
        ...
        bytes += byte_counter;
        [armData appendBytes:amrFrame
                      length:sizeof (unsigned char) * byte_counter];
