...
- (void)logIn
{
    ...
    else
    {
        ...
        if (![TBSocialShareConfig instance].isCloseWeiBoSSO) {
            // open sina weibo app
        ...
        if (!ssoLoggingIn)
        {
            ...
            TBSocialShareWebViewController *authorize = [[TBSocialShareWebViewController alloc]
                    initWithAuthParams:params delegate:self];
...
- (void)saveAuthorizeDataToKeychain {
    ...
    NSString *strDate = [dateFormatter stringFromDate:self.expirationDate];
...
- (void)readAuthorizeDataFromKeychain {
    NSString *serviceName = [self urlSchemeString];
    self.userID = [SFHFKeychainUtils getPasswordForUsername:kWBKeychainUserID
                                                   andServiceName:serviceName
                                                            error:nil];
    self.accessToken = [SFHFKeychainUtils getPasswordForUsername:kWBKeychainAccessToken
                                                        andServiceName:serviceName
                                                                 error:nil];
    NSString *strDate = [SFHFKeychainUtils getPasswordForUsername:kWBKeychainExpireTime
                                                   andServiceName:serviceName
                                                            error:nil];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    self.expirationDate = [dateFormatter dateFromString:strDate];
    [dateFormatter release];
}
...
- (void)deleteAuthorizeDataInKeychain {
    ...
    NSString *serviceName = [self urlSchemeString];
    [SFHFKeychainUtils deleteItemForUsername:kWBKeychainUserID
                              andServiceName:serviceName
                                       error:nil];
    [SFHFKeychainUtils deleteItemForUsername:kWBKeychainAccessToken
                              andServiceName:serviceName
                                       error:nil];
    [SFHFKeychainUtils deleteItemForUsername:kWBKeychainExpireTime
                              andServiceName:serviceName
                                       error:nil];
}
