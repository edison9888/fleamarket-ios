...
- (BOOL)isBlank {
    return [TBRONSStringUtil isBlank:self];
}
...
- (NSString *)stripStrangeWords {
    /* 当输入中文时未完成，直接提交，需要过滤这个奇怪的字符 */
    char mark[4] = { 0xe2, 0x80, 0x86, '\0' };
    NSString *divstr = [NSString stringWithUTF8String:(const char *)mark];
    return [self stringByReplacingOccurrencesOfString:divstr withString:@""];
}
